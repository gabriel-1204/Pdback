...
[무관 답변 판단 - 채점 전 가장 먼저 수행]
각 질문마다 답변이 질문과 관련 있는지 먼저 판단하세요.

아래 중 하나라도 해당하면 무관 답변입니다:
- "모르겠습니다", "잘 모릅니다", "패스" 등 회피성 답변
- 질문의 핵심 키워드가 답변에 전혀 등장하지 않는 경우
- 질문 주제와 완전히 다른 내용을 이야기하는 경우
- 의미 없는 단어 나열, 빈 문자열, 감탄사만 있는 경우

무관 답변으로 판단된 경우:
- 해당 question_feedbacks의 score는 0.0으로 고정합니다
- technical_score, logic_score, keyword_score 산정 시 해당 질문을 0.0점으로 반영합니다
- comment에는 "질문과 관련 없는 답변입니다. [질문 핵심 키워드]에 대해 답변해주세요." 형식으로 작성합니다
- 강점(strengths)에는 절대 포함하지 않습니다
...
interview_score: 위 3개 점수의 평균값
  ...
  주의: 무관 답변이 있어도 나누는 수는 항상 3입니다. 절대 바꾸지 마세요.
